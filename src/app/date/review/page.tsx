...
function ReviewContent() {
  ...
  return <AuthGuard><motion.main initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 space-y-4 px-5 pb-28 pt-6">
    ...
    <section className="pt-1"><button type="button" onClick={save} disabled={!dirty && Boolean(savedAt)} className="flex w-full items-center justify-center gap-2 rounded-xl bg-coral py-3.5 text-sm font-semibold text-white shadow-[0_8px_28px_rgba(255,122,89,0.2)] disabled:bg-navy-light disabled:text-text-secondary disabled:shadow-none">{!dirty && savedAt ? <CheckCircle2 size={18} /> : <Save size={18} />}{!dirty && savedAt ? "保存済み" : "振り返りを保存"}</button><p className="mt-2 text-center text-[10px] text-text-secondary">{savedAt ? `${new Date(savedAt).toLocaleString("ja-JP")} に保存` : "この端末に保存されます"}</p></section>
  </motion.main>{notice ? <div role="status" className="fixed left-1/2 top-5 z-[70] -translate-x-1/2 rounded-full bg-teal px-4 py-2 text-xs font-semibold text-navy shadow-xl">振り返りを保存しました</div> : null}<BottomTabBar /></AuthGuard>;
}
