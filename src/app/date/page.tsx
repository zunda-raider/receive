...
function ChoiceSection({
  ...
  return (
    <section className="rounded-[20px] border border-border-subtle bg-navy-card p-4">
  ...
  return (
    <AuthGuard>
      <motion.main initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 space-y-4 px-5 pb-28 pt-6">
        ...
        <section className="overflow-hidden rounded-[22px] border border-teal/30 bg-gradient-to-br from-teal/10 to-navy-card p-4">
          <div className="flex items-center justify-between">
            <span className="rounded-full bg-teal px-2.5 py-1 text-[10px] font-bold text-navy">{datePlan.date < "2026-08-02" ? "過去のデート" : "予定のデート"}</span>
        ...
        <section className="overflow-hidden rounded-[20px] border border-border-subtle bg-navy-card">
        ...
        <section className="rounded-[20px] border border-border-subtle bg-navy-card p-4">
          ...
          <div className="mt-4 max-h-72 space-y-2 overflow-y-auto rounded-xl bg-navy-light/60 p-3" aria-live="polite">
            {chat.map((message, index) => (
              <div key={`${message.role}-${index}`} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                <p className={`max-w-[88%] rounded-2xl px-3 py-2 text-[11px] leading-relaxed ${message.role === "user" ? "rounded-br-sm bg-coral text-white" : "rounded-bl-sm bg-navy-card text-text-secondary"}`}>{message.text}</p>
              </div>
            ))}
            {isAiTyping ? (
              <div className="flex justify-start" aria-label="AIが回答を考えています">
                <div className="flex items-center gap-1.5 rounded-2xl rounded-bl-sm bg-navy-card px-3 py-2.5">
                  {[0, 1, 2].map((dot) => (
                    <span
                      key={dot}
          ...
          <form onSubmit={submitChat} className="mt-3 flex gap-2">
            ...
            <button type="submit" aria-label="相談を送信" disabled={!chatInput.trim() || isAiTyping} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-teal text-navy disabled:opacity-40"><Send size={16} /></button>
          </form>
        </section>

        <section className="pt-1">
          <button type="button" onClick={savePlan} disabled={!isDirty || isAiTyping} className="flex w-full items-center justify-center gap-2 rounded-xl bg-coral py-3.5 text-sm font-semibold text-white shadow-[0_8px_28px_rgba(255,122,89,0.2)] transition-colors hover:bg-coral-hover disabled:bg-navy-light disabled:text-text-secondary disabled:shadow-none">
      ...
      {notice ? <div role="status" className="fixed left-1/2 top-5 z-[70] -translate-x-1/2 rounded-full bg-teal px-4 py-2 text-xs font-semibold text-navy shadow-xl">{notice}</div> : null}
      <BottomTabBar />
    </AuthGuard>
  );
}
